...
tablename = '_minimum_distance_bed'
lrdata = '_lr_kr_bed'
grdata = '_gr_bed'
...
part_low_vol_array = np.array(part_low_vol_array)
csvarray = np.array(part_high_vol_array)
...
if len(csvarray) > 0:
    part_west = []
    part_east = []
for pos_index in range(0, len(csvarray)):
    if csvarray[pos_index, 7] < 250:
        part_west.append(csvarray[pos_index])
    else:
        part_east.append(csvarray[pos_index])
if len(part_west) > 0:
...
lam = new_part_num / (total_area) 

rE = 1 / (2 * np.sqrt(lam))

R = rA / rE

with open(filelocation + filename + '_R_bed.' + timestep + '.csv', 'w', newline='') as f:
    writer=csv.writer(f)
...
plt.savefig(filelocation + filename + '_lr_bed.' + timestep + '.svg', format='svg')
...
plt.savefig(filelocation + filename + '_gr_bed.' + timestep + '.svg', format='svg')
